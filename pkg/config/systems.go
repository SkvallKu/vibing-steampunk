...
// SAP_USER and SAP_PASSWORD variables. Those usually belong to another system
// (the one in .env, which vsp loads for every process started in that
// directory), and a server started for this system must not log on to it when
// this system's own settings are missing.
...
// tooling (SAP_USER/SAP_PASSWORD), which is not tied to a system.
